...
class GameManager {
  ...
  async init() {
    ...
    const nickname = params.get('nickname') || '玩家';
    this.gameState.isHost = params.get('isHost') === 'true';
    const isCreating = params.get('isCreating') === 'true';

    console.log(`[Game] ====== 页面初始化开始 ======`);
    console.log(`[Game] URL: ${window.location.href}`);
    console.log(`[Game] roomId=${this.gameState.roomId}, nickname=${nickname}, isHost=${this.gameState.isHost}, isCreating=${isCreating}`);

    if (this.gameState.roomId) {
      roomUI.updateRoomId(this.gameState.roomId);
      console.log(`[Game] 房间号已显示: ${this.gameState.roomId}`);
    } else {
      console.warn('[Game] URL中没有roomId参数!');
    }
    ...
    roomUI.showGameStatus('正在连接', '正在连接服务器...');
    this.bindEvents();

    const socketPromise = this.initSocket().then(() => {
      console.log('[Game] ✅ Socket初始化完成');
    }).catch(error => {
      console.error('[Game] ❌ Socket初始化失败:', error?.message || error);
      roomUI.showGameStatus('连接失败', `无法连接服务器 (${error?.message || '未知错误'})，部分功能受限`);
    });

    const voicePromise = this.initVoice().then(() => {
      console.log('[Game] ✅ 语音初始化完成');
    }).catch(error => {
      console.warn('[Game] ⚠️ 语音初始化失败（不影响游戏）:', error?.message || error);
    });

    await Promise.all([socketPromise, voicePromise]);
    roomUI.hideGameStatus();
    console.log(`[Game] ====== 初始化完成, socketConnected=${socketClient.isConnected} ======`);
  }

  async initSocket() {
    console.log('[Game][Socket] 开始初始化Socket连接...');
    
    try {
      console.log(`[Game][Socket] 正在连接 ${config.serverUrl} ...`);
      await socketClient.connect();
      
      console.log(`[Game][Socket] 连接成功, id=${socketClient.getSocketId()}, connected=${socketClient.isConnected}`);
      ...
      console.error('[Game][Socket] ❌ 初始化异常:', error?.message || error, error?.stack || '');
      this.initLocalMode(new URLSearchParams(window.location.search).get('nickname') || '玩家');
      throw error;
    }
  }

  initLocalMode(nickname) {
    console.log(`[Game] 进入本地模式, roomId=${this.gameState.roomId}, nickname=${nickname}`);
  ...
  async initVoice() {
    console.log('[Game] 开始初始化语音...');
    
    try {
      const initSuccess = await agoraVoice.initialize();
      
      if (!initSuccess) {
        console.warn('[Game] 语音初始化失败（App ID未配置）');
        roomUI.updateVoiceButton(false);
        return;
      }
      
      const joinSuccess = await agoraVoice.joinChannel(
        this.gameState.roomId, 
        Math.random().toString(36).substr(2, 9)
      );
      
      if (joinSuccess) {
        console.log('[Game] ✅ 语音连接成功');
        roomUI.updateVoiceButton(true);
      } else {
        console.warn('[Game] ⚠️ 语音加入频道失败');
        roomUI.updateVoiceButton(false);
      }
    } catch (error) {
      console.error('[Game] ❌ 语音初始化异常:', error);
      roomUI.updateVoiceButton(false);
    }
  }
  ...
  bindEvents() {
    document.getElementById('voice-toggle').addEventListener('click', async () => {
      console.log('[Game] 点击语音按钮...');
      const success = await agoraVoice.toggleMicrophone();
      
      if (success) {
        // 成功：根据是否已连接和麦克风状态更新按钮
        if (agoraVoice.isConnected()) {
          roomUI.updateVoiceButton(agoraVoice.isMicrophoneEnabled());
        } else {
          // 刚刚成功连接
          roomUI.updateVoiceButton(true);
        }
        console.log(`[Game] 语音操作成功, 连接状态: ${agoraVoice.isConnected()}, 麦克风: ${agoraVoice.isMicrophoneEnabled()}`);
      } else {
        // 失败：显示断开状态
        roomUI.updateVoiceButton(false);
        console.warn('[Game] 语音操作失败');
      }
    });

    document.getElementById('bet-clear').addEventListener('click', () => {
  ...
  updateGameState(gameState) {
    ...
    console.log(`[前端状态] phase=${phase}, currentPlayer=${gameState.currentPlayer}, myId=${this.gameState.myPlayerId}, isMyTurn=${gameState.currentPlayer === this.gameState.myPlayerId}, canPlay=${canPlay}, socketConnected=${socketClient.isConnected}`);
...
window.addEventListener('DOMContentLoaded', () => {
  console.log('[Game] DOMContentLoaded 触发, 开始创建GameManager');
