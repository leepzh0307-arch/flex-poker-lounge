...
class RoomUI {
  ...
  updateVoiceButton(enabled) {
    if (!this.elements.voiceToggle) return;
    
    const isConnected = window.agoraVoice && window.agoraVoice.isConnected();
    const isMicEnabled = window.agoraVoice && window.agoraVoice.isMicrophoneEnabled();
    
    let text, isActive, title;
    ...
    this.elements.voiceToggle.textContent = text;
    this.elements.voiceToggle.classList.toggle('active', isActive);
    this.elements.voiceToggle.title = title;
    
    console.log(`[UI] 语音按钮更新: ${text}, 连接=${isConnected}, 麦克风=${isMicEnabled}`);
  }
