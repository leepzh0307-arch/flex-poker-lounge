    ...
    this.connectPromise = new Promise((resolve, reject) => {
      ...
      try {
        ...
        this.socket.on('connect', () => {
          console.log(`[Socket] ✅ 连接成功! id=${this.socket.id}, transport=${this.socket.io.engine.transport.name}`);
        ...
        this.socket.on('connect_error', (error) => {
          ...
          console.error(`[Socket] 后端地址: ${url}`);
          console.error(`[Socket] 可能原因: 1.Render服务休眠中 2.Cloudflare未开启WebSocket 3.网络问题`);
          this.isConnected = false;
          if (!resolved) { resolved = true; reject(error); }
        });

        this.socket.on('disconnect', (reason) => {
          console.log(`[Socket] 断开连接: ${reason}, transport=${this.socket.io?.engine?.transport?.name || 'unknown'}`);
          this.isConnected = false;
        });

        this.socket.io.engine.on('upgrade', (transport) => {
          console.log(`[Socket] 传输升级: ${transport.name}`);
        });
